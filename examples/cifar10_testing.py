...
if __name__ == '__main__':
    ...
    model_test = resnet50(num_classes=10).to(device)
    model_test = load_model(model_test, 'resnet50_cifar10_test.pth')
    model_test.eval()

    model_hsv = resnet50(num_classes=10).to(device)
    model_hsv = load_model(model_hsv, 'advtrain_cifar10_HSV.pth')
    model_hsv.eval()

    model_rt = resnet50(num_classes=10).to(device)
    model_rt = load_model(model_rt, 'advtrain_cifar10_RT.pth')
    model_rt.eval()

    model_edgefool = resnet50(num_classes=10).to(device)
    model_edgefool = load_model(model_edgefool, 'advtrain_cifar10_EdgeFool.pth')
    model_edgefool.eval()

    model_ensemble = resnet50(num_classes=10).to(device)
    model_ensemble = load_model(model_ensemble, 'advtrain_cifar10_triple.pth')
    model_ensemble.eval()
    ...
    sat.test(model_edgefool, testloader_clean, criterion, device, t=get_tqdm('Clean testset - EdgeFool AdvTrain', data_len, True))
    sat.test(model_ensemble, testloader_clean, criterion, device, t=get_tqdm('Clean testset - Ensemble AdvTrain', data_len, True))
    ...
    hsv_set = load_adv_list('advlist_cifar10_hsv_test_no_ro.pth')
    #hsv_set = advtrain.generate(testloader,
    #                            early_stopping=True,
    #                            num_adv=10000,
    #                            to_file=True,
    #                            file_name='advlist_cifar10_hsv_test_all.pth',
    #                            resume=False)
    ...
    advtrain.test(model_edgefool, hsv_set, should_generate=False, batch_size=256, t_desc='HSV testset - EdgeFool AdvTrain')
    advtrain.test(model_ensemble, hsv_set, should_generate=False, batch_size=256, t_desc='HSV testset - Ensemble AdvTrain')
    ...
    rt_set = load_adv_list('advlist_cifar10_rt_test_no_ro.pth')
    #rt_set = advtrain.generate(testloader,
    #                           early_stopping=True,
    #                           num_adv=10000,
    #                           to_file=True,
    #                           file_name='advlist_cifar10_rt_test_all.pth',
    #                           resume=False)
    ...
    advtrain.test(model_edgefool, rt_set, should_generate=False, batch_size=256, t_desc='RT testset - EdgeFool AdvTrain')
    advtrain.test(model_ensemble, rt_set, should_generate=False, batch_size=256, t_desc='RT testset - Ensemble AdvTrain')
    ...
    edgefool_set = load_adv_list('advlist_cifar10_edgefool_test_no_ro.pth')
    #edgefool_set = advtrain.generate(testloader,
    #                                 early_stopping=True,
    #                                 num_adv=10000,
    #                                 to_file=True,
    #                                 file_name='advlist_cifar10_edgefool_test_all.pth',
    #                                 resume=False)

    advtrain.test(model_test, edgefool_set, should_generate=False, batch_size=256, t_desc='EdgeFool testset - No AdvTrain')
    advtrain.test(model_hsv, edgefool_set, should_generate=False, batch_size=256, t_desc='EdgeFool testset - HSV AdvTrain')
    advtrain.test(model_rt, edgefool_set, should_generate=False, batch_size=256, t_desc='EdgeFool testset - RT AdvTrain')
    advtrain.test(model_edgefool, edgefool_set, should_generate=False, batch_size=256, t_desc='EdgeFool testset - EdgeFool AdvTrain')
    advtrain.test(model_ensemble, edgefool_set, should_generate=False, batch_size=256, t_desc='EdgeFool testset - Ensemble AdvTrain')
