...
import json
import torch
#mport lib.utils.data as torchdata
import cv2
import torchvision
...
class DatasetBuilder:
    ...
    def build(self, datasetString):

        dataset = None
        if datasetString == 'mnist':
            dataset = torchvision.datasets.MNIST(root='./data', transform=transform, download=True)
        elif datasetString == 'cifar10':
            dataset = torchvision.datasets.CIFAR100(root='./data', transform=transform, download=True)
